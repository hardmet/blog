...
<page:template>
    ...
    <jsp:attribute name="scrypt">
        ...
        <script src="<c:url value="/resources/js/post_ws.js" />"></script>
                        ...
                        <c:if test="${!empty postRelatedData.post}">
                            ...
                            <span class="meta post-time">Posted by
                                    ...
                                    on <fmt:formatDate type="date" dateStyle="long" pattern="MMMM dd, yyyy"
                                                       value="${postRelatedData.post.date}"/></span>
                            <span id="postId" style="display: none;">${postRelatedData.post.id}</span>
                            <c:if test="${isUSer}">
                                <security:authentication property="principal.username" var="userName"/>
                                <span id="currentSessionUser" style="display: none;">${userName}</span>
                            </c:if>
                            <c:if test="${not isUSer}">
                                <span id="currentSessionUser" style="display: none;"></span>
                            </c:if>
                        </c:if>
                    </div>
                            ...
                            <c:if test="${!empty postRelatedData.comments}">
                                <c:forEach items="${postRelatedData.comments}" var="comment">
                                    <div class="comment-content" id="comment_${comment.id}">
                                        <span class="comment-head">${comment.author}
                                            <span class="fa fa-close delete-content" title="удалить комментарий"></span>
                                            <c:if test="${not isUniqUser}">
                                                <script>
                                                    $(".delete-content").css("display", "none");
                                                </script>
                                            </c:if>
                                            <span class="comment-time"><fmt:formatDate type="date" dateStyle="long"
                                                                                       pattern="MMMM dd, yyyy"
                                                                                       value="${comment.date}"/></span>
                                        </span>
